...
void YahooStock::OnUrlGetDone(wxUrlGetDoneEvent& event){
    ...
    if (data->rType == REALTIME_RETRIVE){ //Real Time Data come!
        ...
        if (data->FetchSeed == HistoryFetchSeed){
            Stock*s = (Stock*)data->OrignUserData;
            ...
            if (data->StartIdx == 2){
                ...
                FetchHistoryData((Stock*)data->OrignUserData,data->StartIdx+1);
            }
        }
    }
    delete(data);
...
void YahooStock::FetchHistoryData(Stock* s,int datatype){
    wxString keys[]={
        wxT("d"),
        wxT("w"),
        wxT("m"),
...
void YahooStock::RetriveHistoryDayData(Stock* s){
    HistoryFetchSeed++;
    FetchHistoryData(s);
}
